...
// Automatically deduce the main page URL (e.g. index.hbs or main.aspx) from the available browser windows.
...
function GetMainPageUrl()
{
	...
	.then(clients =>
	{
		...
		{
			// Parse off the scope from the full client URL, e.g. https://example.com/index.html -> index.hbs
			let url = c.url;
...
function UpdateCheck(isFirst)
{
	...
	.then(data =>
	{
		...
		.then(cacheExists =>
		{
			...
			// Implicitly add the main page URL to the file list, e.g. "index.hbs", so we don't have to assume a specific name.
